...
func BuildPrompt(input PromptInput) (string, error) {
	...
	if input.Phase == pipeline.PhasePlanning {
		...
		b.WriteString("Use advisory phase bands of exactly 1 for Trivial, usually 1–2 for Simple, usually 2–3 for Moderate, and usually 3–5 for Complex. Only Trivial exactly one and the hard maximum of 6 phases are enforced; do not create artificial splits to satisfy an advisory band. Preserve the complete scope.\n")
		...
		b.WriteString("Representative benchmark: a README-only wording update is Trivial with exactly one phase; a localized backward-compatible bug fix is Simple and normally one to two phases; an ordered multi-component feature is Moderate and normally two to three phases; a cross-service or breaking migration is Complex and normally three to five phases.\n")
		b.WriteString("Never truncate, merge, rename, or drop requested scope to fit the limit. If a plan would exceed six phases, consolidate cohesive work while preserving the complete outcome before writing the artifact.\n")
		...
		if attempt > 1 {
			b.WriteString("\n## Planning correction\n")
			fmt.Fprintf(&b, "This is Planning attempt %d of %d. A fresh agent is correcting the rejected artifact below; preserve the complete original goal and acceptance criteria above. The six-phase maximum remains a hard cap.\n", attempt, MaxPlanningAttempts)
