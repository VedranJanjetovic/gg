...
func writeCommandHelp(w io.Writer, command string) {
	...
	if command == "run" {
		fmt.Fprint(w, "\nRun controls: --parent-branch, --base-ref, --enable-pr, --disable-pr, --enable-ci, --disable-ci, and --max-iterations (default 10; the absolute ceiling of QA attempts — the loop parks earlier when the same finding keeps recurring). Add --repair-existing-verification to let Development repair the verification failures that already existed on the parent branch; without it those failures are recorded as warnings and only new regressions block the run. New projects choose Inherit or Pick configuration in the attached TUI; use -- to pass every following token to the pipeline unchanged.\n")
	}
	if command == "resume" {
