...
description: "Disciplined single-change delivery: understand, branch, implement, test, review-loop, commit"
...
4. Plan the change and spawn an implementation subagent with the framed problem and Go implementation workflow.
...
7. Run an implementation/review remediation loop until no confirmed fixable issues remain. The reviewer must verify that tests exist and are valid.
...
Then ALWAYS spawn exactly one implementation subagent using: /go-developer
...
Spawn exactly one testing subagent using: /test
...
Before proposing a commit or push, run a review loop. Each review pass must spawn exactly one independent reviewer subagent using: /review
...
2. Spawn exactly one implementation subagent using /go-developer and pass the remediation brief.
...
1. Draft the commit message using
...
9. Always spawn exactly one implementation subagent with the framed problem, implementation plan, and Go implementation workflow.
...
19. Always run the implementation/review remediation loop before proposing commit/push.
...
7. An implementation subagent applied the Go implementation workflow.
...
15. The implementation/review remediation loop ran until no confirmed fixable issues remained.
