---
name: gg-multi-phase-workflow
description: "Plan, execute, and track multi-phase projects across isolated worktrees"
argument-hint: "<problem description> | execute | progress | close [id]"
allowed-tools:
  - Read
  - Glob
  - Grep
...
Orchestrator for complex changes that span multiple deliverables. Splits work into phases, executes them in parallel where possible via `gg-single-phase-workflow`, and tracks state across sessions.
...
- If starts with `execute`: run Execute flow
- If starts with `progress`: run Progress flow
- If starts with `close`: run Close flow (optional project-id after)
- Otherwise: treat as a new problem description, run Plan flow

## Subcommands

### `/gg-multi-phase-workflow <problem description>`

Plan a new multi-phase project.

### `/gg-multi-phase-workflow execute`

Pick a non-finished project and execute the next batch of ready phases.

### `/gg-multi-phase-workflow progress`

Show all projects and their phase statuses. User manually marks phases as done.

### `/gg-multi-phase-workflow close [project-id]`

Clean up: remove project files, worktrees, and branches.

---

## Subcommand: Plan (`/gg-multi-phase-workflow <problem>`)
...
## Subcommand: Execute (`/gg-multi-phase-workflow execute`)
...
3. Spawn a `/gg-single-phase-workflow` subagent for the phase with:
...
## Subcommand: Progress (`/gg-multi-phase-workflow progress`)
...
## Subcommand: Close (`/gg-multi-phase-workflow close [project-id]`)
...
3. Always use `/gg-single-phase-workflow` (or equivalent) for phase execution — do not implement inline.
